(ns webmk.app
  "webmk-wbmk0001 appview — reagent + re-frame, view built from
  jp-go-dds (デジタル庁デザインシステム) hiccup.

  This is a faithful port of the previous SvelteKit scaffold
  (`svelte/src/routes/+page.svelte`, originally
  `60-apps/etzhayyim-project-webmk/appview/webmk-wbmk0001/svelte/src/routes/+page.svelte`):
  a static info page describing this appview surface itself — its
  ...
  dispatcher or the kotoba/ reference slice — it was pure static markup
  describing the surface's own metadata, and this port adds no
  web-marketing-agent functionality that was not already there.
  ...
  `tia-mcp-component/cljs`, itself copied from
  `cloud-itonami/app-scheduler`'s `scheduler-mcp-component/cljs`).
...
(def default-db
  {:title "Webmk Wbmk0001"
   ...
   :xrpc? true
   :relative-path "appview/webmk-wbmk0001/cljs/src/webmk/app.cljs"})
